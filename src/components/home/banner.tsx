...
const Banner = () => {
	...
	return (
		<AnimatePresence mode="wait">
			...
			>
				<img
					src={`https://phimapi.com/image.php?url=${dataFilm.thumb_url}`}
					alt=""
				...
				<AnimatePresence mode="wait">
					...
					>
						<div className="name text-[3rem] text-[#FFD875]">{dataFilm.name || 'Loading...'}</div>
						...
						<ul className="flex items-center gap-[0.5rem] mt-[0.6rem] mb-[24px]">
							{dataFilm?.category?.map((item: any, index: number) => (
								<li key={index} className="cursor-pointer" onClick={() => handleClick(item.slug)}>
    ...
    ${activeSlug === item.slug ? 'border-[#fff] scale-[1.05] border-[3px]' : 'border-[#ffffff24] scale-0.1]'}`}
							>
								<img
